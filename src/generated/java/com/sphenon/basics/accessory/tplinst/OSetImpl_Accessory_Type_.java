...
public class OSetImpl_Accessory_Type_
  implements OSet_Accessory_Type_ {
    protected java.util.Hashtable map;
    protected Set_Accessory_ toplevel_set;
    static protected Type itemtype;


    public OSetImpl_Accessory_Type_ (CallContext context) {
        if (itemtype == null) { itemtype = TypeManager.get(context, Accessory.class); }
    }

    public OSetImpl_Accessory_Type_ (CallContext context, java.util.Hashtable map) {
        this.map = map;
        if (itemtype == null) { itemtype = TypeManager.get(context, Accessory.class); }
    }
    ...
    protected void initialise(CallContext context) {
        if (this.toplevel_set == null) {
            ...
            this.toplevel_set = new SetImpl_Accessory_ (context);
    ...
    public Set_Accessory_ tryGet  (CallContext context, Type index) {
        ...
        return (Set_Accessory_) (map == null ? null : map.get(index));
    }
    ...
    private Set_Accessory_ getEntry(CallContext context, Type index) {
        initialise(context);
        Set_Accessory_ set = (Set_Accessory_) map.get(index);
        if (set == null) {
            set = new SetImpl_Accessory_ (context);
            map.put(index, set);
        }
        return set;
    }

    private void    _setSuper  (CallContext context, Accessory item, Type index) {
    ...
    private void    _unsetSuper  (CallContext context, Accessory item, Type index) {
        if (index.equals(itemtype)) return;
        // Iterator_Type_ supertypes = index.getSuperTypes(context).getNavigator(context);
        Iterator_Type_ supertypes = index.getAllSuperInterfaces(context).getNavigator(context);
        Type supertype;
